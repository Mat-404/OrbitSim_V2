...
def update_data(frame):
    ...
    for i in config_list.keys():
        ...
        if len(config_list[i]["XVals"]) > 50:
            config_list[i]["XVals"].pop(0)
            config_list[i]["YVals"].pop(0)

        # Get angle between Earth and Mars
        if abs(config_list["Earth"]["Theta"] - config_list["Mars"]["Theta"]) < 44 * (math.pi / 180) and not config_list["Transfer"]["Created"]:   
            config_list["Transfer"]["Apoapsis Radius (km)"] = config_list["Mars"]["SemiMajorAxis"] * (1 - config_list["Mars"]["Eccentricity"] ** 2) / (
                    1 + (config_list["Mars"]["Eccentricity"]) * math.cos(config_list["Mars"]["Theta"]))
...
for i in config_list.keys():
    config_list[i]["Eccentricity"] = (config_list[i]["Apoapsis Radius (km)"] - config_list[i]["Periapsis Radius (km)"]) / (
                config_list[i]["Apoapsis Radius (km)"] + config_list[i]["Periapsis Radius (km)"])
    config_list[i]["SemiMajorAxis"] = (config_list[i]["Apoapsis Radius (km)"] + config_list[i]["Periapsis Radius (km)"]) / 2
    config_list[i]["XVals"] = []
    config_list[i]["YVals"] = []
    config_list[i]["t"] = 0
    config_list[i]["Theta"] = 0
    config_list[i]["Period"] = 2 * math.pi * math.sqrt(
        config_list[i]["SemiMajorAxis"] ** 3 / config_list["Sun"]["Gravitational Parameter (km^3/s^2)"])
